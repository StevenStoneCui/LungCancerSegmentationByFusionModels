        ...
        for batch_size in [8, 16, 32, 64]:
            for sigma in [3, 2, 1]:
                for beta in [1, 2, 3]:
                    ...
                    s = DenseVnet3D.DenseVnet3D(main_input)    
                            ...
                            if x % validation_every_n == 0: 
                                for _ in range(validation_max_iter):
                                    ...
                                    for index in range(len(valProcessed)):
                                        ...
                                        result_seg = np.where(current_Seg>=0.5, 1.0, 0.0)
                                        valLoss.append(dice_loss(cp.asnumpy((valLabelProcessed[index])), result_seg[0, :, :, :, :]))
