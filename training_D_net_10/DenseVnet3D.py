...
def conv_block(input, nb_filter, kernal_size=(3, 3, 3), dilation_rate=1,
    ...
    ''' Apply BatchNorm, selu, 3x3X3 Conv3D, optional bottleneck block and dropout
    Args:
        input: Input tensor
...
def transition_block(input, nb_filter, compression=1.0, weight_decay=1e-4,
                       pool_kernal=(3, 3, 3), pool_strides=(2, 2, 1)):
    ''' Apply BatchNorm, selu 1x1, Conv3D, optional compression, dropout and Maxpooling3D
    Args:
        input: input tensor
        nb_filter: number of filters
...
def transition_up_block(input, nb_filters, compression=1.0,
                          kernal_size=(3, 3, 3), pool_strides=(2, 2, 1),
...
def DenseVnet3D(main_input,
                nb_classes=1,
                encoder_nb_layers=(5, 6, 6),
                growth_rate=(8, 16, 32),
                #dilation_list=(5, 3, 1),
                dilation_list=(1, 1, 1),
                dropout_rate=0.25,
    ...
    for block_idx in range(nb_dense_block):
        ...
        if n_down_channels[block_idx] != None:
            x = tf.keras.layers.Conv3D(n_down_channels[block_idx], 3, strides = 2,
                       kernel_initializer='he_normal',
                       padding='same')(x)
    ...
    x = tf.keras.layers.Concatenate(name='skipWaysConcatenate')([x_level3, x_level2, x_level1])
    ...
    '''
    connect_toInput = tf.keras.layers.concatenate([x, PreviousSeg_input, HintMap_input], axis = -1, name='concat_toInput')
    depthWiseConv = tf.keras.layers.Conv3D(3, 3, padding="same", groups=3, name='depthWiseConv')(connect_toInput)
    x = tf.keras.layers.Conv3D(1, 1, padding="same", name='pointWiseConv')(depthWiseConv)
    ...
    if nb_classes == 1:
        ...
        '''
    # Create model.
    model = tf.keras.Model(main_input, x, name='DenseVnet3D')
    return model
'''
###################----Demo Usages----#############
