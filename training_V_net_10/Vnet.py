...
def myConv(x_in, nf, strides=1, kernel_size = 3):
    ...
    """
    x_out = Conv3D(nf, kernel_size=kernel_size, padding='same', activation='relu', strides=strides)(x_in)
    x_out = BatchNormalization()(x_out)
    x_out = tf.keras.layers.SpatialDropout3D(0.25)(x_out)
    return x_out


def Vnet3dBlock(l, n_feat, depth):
    if RESIDUAL:
        l_in = l
    if depth >= 3:
        d = 3
...
def Vnet3d(main_input):
    ...
    for d in range(depth - 1):
        ...
        layer = Vnet3dBlock(layer, num_filters, d + 1)        
        if d != depth - 1:
            down_list.append(layer)
            layer = myConv(layer, num_filters*2, strides=2)
        
    for d in range(depth-2, -1, -1):
        layer = VnetUpsample(layer, filters[d])
        layer = concatenate([layer, down_list[d]])
        layer = Vnet3dBlock(layer, filters[d] * 2, d + 1)
        '''
    ...
    '''
    layer = Conv3D(1, kernel_size=1, padding='same', use_bias=False)(layer)
